...
export const generatechatcompletion= async (req, res, next): Promise<void> => {
...
export const sendchatstouser = async (req:Request, res:Response, next:NextFunction): Promise<void> => {
    try {
      //user login
      const user = await User.findById(res.locals.jwtData.id);
      if (!user) {
...
export const deletecharts = async (req:Request, res:Response, next:NextFunction): Promise<void> => {
    try {
      //user login
      const user = await User.findById(res.locals.jwtData.id);
      if (!user) {
